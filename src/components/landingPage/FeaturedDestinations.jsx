...
// Location tags for Baguio places
const locationTags = {
  'burnham-park': { text: 'Most Popular', color: 'gold' },
  'session-road': { text: 'Shopping Hub', color: 'blue' },
  'mines-view-park': { text: 'Best View', color: 'red' },
  'strawberry-farm': { text: 'Unique Experience', color: 'green' },
  'teachers-camp': { text: 'Historical Gem', color: 'blue' },
  'sm-baguio': { text: 'Shopping Favorite', color: 'gold' },
  'baguio-cathedral': { text: 'Heritage Site', color: 'blue' },
  'wright-park': { text: 'Adventure Pick', color: 'green' }
};
...
const FeaturedDestinations = ({ onNavigate }) => {
  ...
  const totalPages = Math.ceil(baguioData.locations.length / ITEMS_PER_PAGE);
  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
  const endIndex = startIndex + ITEMS_PER_PAGE;
  const currentLocations = baguioData.locations.slice(startIndex, endIndex);
  ...
  return (
    <section ref={ref} className="featured-destinations">
      ...
      <div className="destinations-container">
        ...
        {/* Map Preview */}
        <MapPreview
          locations={baguioData.locations}
